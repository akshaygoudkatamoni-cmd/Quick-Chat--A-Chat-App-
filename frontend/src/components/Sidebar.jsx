...
const SideBar = () => {
    ...
    const filterUsers = () => {
        if(input){
            return users.filter(user => user.fullName.toLowerCase().includes(input.toLowerCase()))
        }
        return users
    }
        ...
        {filterUsers().map((user, index) => (
            <div onClick={() => {setSelectedUser(user); setUnseenMessages(prev => ({...prev, [user._id]: 0}))}}
             key={user._id || index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm ${selectedUser?._id === user._id && 'bg-[#282142]/50'}`}>
                <img src={user?.profilePic || assets.avatar_icon} alt="" className='w-[35px] h-[35px] object-cover rounded-full' />
                ...
                {String(selectedUser?._id) !== String(user._id) && unseenMessages?.[user._id] > 0 && (
